...
export function useRound() {
  ...
  // Tracks the timestamp of the most recent local edit. Used by the realtime
  // subscription below to ignore echoes that arrive while the user is actively tapping.
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          if (!next) return;
          // Echo guard: if a local edit happened within the last 2 seconds, the incoming
          // payload is likely the echo of our own debounced save. Applying it could overwrite
          // a fresher local tap that came after the save fired. Skip until things settle.
          const sinceLastEdit = Date.now() - lastLocalEditRef.current;
